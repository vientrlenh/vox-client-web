  ...
  return (
    <div className="grid gap-3 rounded-lg border border-slate-200 bg-white p-4">
      ...
      <div className="flex items-center justify-between text-sm font-semibold text-slate-600">
        <span>
          {classesQuery.data?.totalElements ?? 0} lớp học, trang {(classesQuery.data?.page ?? 0)}/{classesQuery.data?.totalPages ?? 1}
        </span>
        <div className="flex gap-2">
          ...
          <button
            className="h-9 rounded-lg border border-slate-200 px-3 transition hover:bg-slate-50 disabled:opacity-50"
            disabled={page >= (classesQuery.data?.totalPages ?? 1)}
            onClick={() => setPage((p) => p + 1)}
            type="button"
          >
            Sau
...
function ClassPickerForTeacher({ onChange, value, schoolId }: { onChange: (id: string) => void; value: string; schoolId: string }) {
  const [search, setSearch] = useState('')
  const [page, setPage] = useState(1)
  const classesQuery = useMySchoolClassesQuery(page, 10, schoolId, 'ACTIVE')
  return <ClassPickerTable classesQuery={classesQuery} onChange={onChange} page={page} search={search} setPage={setPage} setSearch={setSearch} value={value} />
}

function ClassPickerForAdmin({ onChange, value }: { onChange: (id: string) => void; value: string }) {
  const [search, setSearch] = useState('')
  const [page, setPage] = useState(1)
...
function ClassPicker({
  ...
  if (isTeacher) {
    return <ClassPickerForTeacher onChange={onChange} schoolId={user?.schoolId ?? ''} value={value} />
  }
  return <ClassPickerForAdmin onChange={onChange} value={value} />
}
