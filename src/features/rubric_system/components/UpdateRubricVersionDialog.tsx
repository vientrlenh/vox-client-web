    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-0">
      ...
      <div className="relative w-full max-w-2xl rounded-xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <h2 className="text-lg font-bold text-slate-900">Chỉnh sửa Version</h2>
        ...
        <form onSubmit={handleSubmit} className="max-h-[80vh] overflow-y-auto p-6">
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            <div className="sm:col-span-2">
              ...
              <select value={formData.totalScoreMethod ?? ''} onChange={(e) => setFormData({ ...formData, totalScoreMethod: e.target.value })} disabled={isPending} required className="w-full rounded-lg border border-slate-300 px-4 py-2 text-sm outline-none transition focus:border-cyan-500 disabled:bg-slate-50">
            ...
            <div>
              ...
              <input type="date" value={formData.effectiveFrom ?? ''} onChange={(e) => setFormData({ ...formData, effectiveFrom: e.target.value })} disabled={isPending} required className="w-full rounded-lg border border-slate-300 px-4 py-2 text-sm outline-none transition focus:border-cyan-500 disabled:bg-slate-50" />
            </div>

            <div>
              <label className="mb-1 block text-sm font-bold text-slate-700">Đến ngày (Tùy chọn)</label>
              <input type="date" value={formData.effectiveTo ?? ''} onChange={(e) => setFormData({ ...formData, effectiveTo: e.target.value })} disabled={isPending} className="w-full rounded-lg border border-slate-300 px-4 py-2 text-sm outline-none transition focus:border-cyan-500 disabled:bg-slate-50" />
            </div>

            {/* Mô tả: Chiếm trọn 2 cột */}
            <div className="sm:col-span-2">
              <label className="mb-1 block text-sm font-bold text-slate-700">Mô tả chi tiết</label>
              <textarea rows={3} value={formData.description ?? ''} onChange={(e) => setFormData({ ...formData, description: e.target.value })} disabled={isPending} className="w-full rounded-lg border border-slate-300 px-4 py-2 text-sm outline-none transition focus:border-cyan-500 disabled:bg-slate-50" />
            </div>
          </div>
