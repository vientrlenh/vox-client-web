...
export function UpdateRubricResultBandDialog({ isOpen, onClose, onSubmit, isPending, initialData, existingOrders = [] }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (existingOrders.includes(Number(formData.order))) {
      setErrorMessage(`Lỗi: Thứ tự (Order) ${formData.order} đã được sử dụng bởi một thang điểm khác trong phiên bản này. Vui lòng chọn thứ tự khác.`);
      return;
    }
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            <div>
              ...
              <input type="number" step="0.01" value={formData.scoreMin} onChange={(e) => setFormData({ ...formData, scoreMin: e.target.value === '' ? 0 : Number(e.target.value) })} disabled={isPending} required className="w-full rounded-lg border border-slate-300 px-4 py-2 text-sm outline-none transition focus:border-indigo-500 disabled:bg-slate-50" />
            </div>

            <div>
              <label className="mb-1 block text-sm font-bold text-slate-700">Điểm tối đa (Max)</label>
              <input type="number" step="0.01" value={formData.scoreMax} onChange={(e) => setFormData({ ...formData, scoreMax: e.target.value === '' ? 0 : Number(e.target.value) })} disabled={isPending} required className="w-full rounded-lg border border-slate-300 px-4 py-2 text-sm outline-none transition focus:border-indigo-500 disabled:bg-slate-50" />
            </div>
