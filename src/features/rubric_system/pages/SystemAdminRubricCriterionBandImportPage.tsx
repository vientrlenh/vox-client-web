// src/features/rubric_system/pages/SystemAdminRubricCriterionBandImportPage.tsx
...
import { usePreviewSystemRubricCriterionBandImportMutation } from '../api/usePreviewSystemRubricCriterionBandImportMutation';
import { useAcceptSystemRubricCriterionBandImportMutation } from '../api/useAcceptSystemRubricCriterionBandImportMutation';
import { searchRubricCriterionBandKeys } from '../api/useSearchSystemRubricCriterionBandsQuery';
...
function createInitialMapping(preview: PreviewRubricCriterionBandImportResponse) {
...
type MappingPanelProps = {
  ...
  preview: PreviewRubricCriterionBandImportResponse;
};
...
export function SystemAdminRubricCriterionBandImportPage() {
  const { rubricId, versionId, criterionId } = useParams<{ rubricId: string; versionId: string; criterionId: string }>();
  const navigate = useNavigate();
  const queryClient = useQueryClient();

  const previewMutation = usePreviewSystemRubricCriterionBandImportMutation(criterionId);
  const acceptMutation = useAcceptSystemRubricCriterionBandImportMutation();

  const [preview, setPreview] = useState<PreviewRubricCriterionBandImportResponse | null>(null);
  ...
  async function handleAccept() {
    ...
    try {
      ...
      await queryClient.invalidateQueries({ queryKey: searchRubricCriterionBandKeys.all });
      ...
      navigate(buildImportSessionDetailPath('/system-admin', preview.sessionId), {
        state: {
          invalidateKeys: [searchRubricCriterionBandKeys.all],
          returnLabel: 'Quay lại tiêu chí',
          returnTo: `/system-admin/rubrics/${rubricId ?? ''}/versions/${versionId ?? ''}/criteria/${criterionId ?? ''}`,
        } satisfies ImportSessionNavState,
      });
    } catch (error) {
      setMessage({
        text: getErrorMessage(error) ?? 'Không thể xác nhận import mức điểm. Vui lòng thử lại.',
        tone: 'error',
      });
    }
  }
  ...
  const backToCriterionUrl = `/system-admin/rubrics/${rubricId ?? ''}/versions/${versionId ?? ''}/criteria/${criterionId ?? ''}`;

  return (
    <section aria-labelledby="system-rubric-criterionband-import-title" className="grid gap-6 font-['Be_Vietnam_Pro',sans-serif] text-slate-950">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <nav aria-label="Đường dẫn" className="flex items-center gap-2 text-sm font-bold text-slate-500">
            <Link className="transition hover:text-indigo-600" to={backToCriterionUrl}>
              Chi tiết Tiêu chí
            </Link>
            <span aria-hidden="true" className="text-slate-300">/</span>
            <span className="text-slate-950">Import Mức điểm hàng loạt</span>
          </nav>
          <h1 className="mt-4 text-3xl font-bold tracking-tight text-slate-950" id="system-rubric-criterionband-import-title">
            Import Mức điểm hàng loạt
          </h1>
          <p className="mt-3 max-w-2xl text-base leading-7 text-slate-500">
            Tải lên file CSV hoặc Excel, kiểm tra mapping cột và xác nhận import các Mức điểm (Criterion Band) mới cho Tiêu chí này.
          </p>
        </div>
        <Link
      ...
      <section className="grid gap-5 rounded-[14px] border border-slate-200 bg-white p-6">
        ...
        <label className="grid cursor-pointer place-items-center gap-3 rounded-[14px] border-[1.5px] border-dashed border-slate-300 bg-slate-50 px-4 py-10 text-center transition hover:border-indigo-400 hover:bg-indigo-50/40">
          ...
          <span className="max-w-md text-[13px] leading-6 text-slate-500">
            File cần có Mã Mức độ, Điểm tối thiểu và Điểm tối đa.
          </span>
          <input
            accept=".csv,.xlsx,.xls"
            className="sr-only"
