// src/features/assessment_policy_school/pages/SchoolAdminAssessmentPolicyImportPage.tsx
...
// Field khớp với AssessmentPolicyImportCommitHandler ở BE. Ba field
// schoolClass/schoolGrade/schoolGradeLevel chỉ áp dụng cho import có schoolId
// (School Admin) để giới hạn phạm vi áp dụng hẹp hơn toàn trường — đều tùy chọn.
const IMPORT_FIELDS: ImportField[] = [
  ...
  { isRequired: false, label: 'Niên khóa', value: 'schoolGrade' },
...
function createInitialMapping(preview: PreviewAssessmentPolicyImportResponse) {
...
type MappingPanelProps = {
  ...
  preview: PreviewAssessmentPolicyImportResponse;
};
...
export function SchoolAdminAssessmentPolicyImportPage() {
  const navigate = useNavigate();
  const queryClient = useQueryClient();
  const user = useAppSelector((state) => state.auth.user);
  const schoolId = user?.schoolId;

  const previewMutation = usePreviewSchoolAssessmentPolicyImportMutation(schoolId);
  const acceptMutation = useAcceptSchoolAssessmentPolicyImportMutation(schoolId);
  ...
  const backToListUrl = '/school-admin/assessment-policies';

  return (
    <section aria-labelledby="school-assessment-policy-import-title" className="grid gap-6 text-slate-950">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <h1 className="mt-4 text-2xl font-black text-blue-950 sm:text-3xl" id="school-assessment-policy-import-title">
      ...
      <section className="grid gap-5 rounded-[14px] border border-slate-200 bg-white p-6">
        ...
        <label className="grid cursor-pointer place-items-center gap-3 rounded-[14px] border-[1.5px] border-dashed border-slate-300 bg-slate-50 px-4 py-10 text-center transition hover:border-indigo-400 hover:bg-indigo-50/40">
          ...
          <span className="max-w-md text-[13px] leading-6 text-slate-500">
            File cần có ngôn ngữ, phiên bản khung, phiên bản rubric, band mục tiêu/tối thiểu và ngày bắt đầu.
          </span>
          <input
            accept=".csv,.xlsx,.xls"
            className="sr-only"
      ...
      {preview ? (
        <>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <SummaryCard label="Hết hạn" value={formatAssessmentPolicyDate(preview.expiresAt)} />
          </div>
