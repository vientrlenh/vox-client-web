...
function createInitialMapping(preview: PreviewSchoolUserImportResponse) {
...
type MappingPanelProps = {
  ...
  preview: PreviewSchoolUserImportResponse
}
...
export function SchoolAdminSchoolUserImportPage() {
  ...
  return (
    <section
      aria-labelledby="school-user-import-title"
      ...
      <section className="grid gap-4 rounded-lg border border-slate-200 bg-white p-5 shadow-sm shadow-slate-950/5">
        ...
        <label className="grid cursor-pointer place-items-center gap-3 rounded-lg border border-dashed border-slate-300 bg-slate-50 px-4 py-8 text-center transition hover:border-cyan-400 hover:bg-cyan-50">
          ...
          <span className="text-xs font-semibold text-slate-500">
            File cần có email, họ tên, vai trò, SĐT, ngày sinh, ngày bắt đầu,
            ngày kết thúc và địa chỉ.
          </span>
          <input
            accept=".csv,.xlsx,.xls"
            className="sr-only"
      ...
      {preview ? (
        <>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <SummaryCard
              label="Hết hạn"
              value={formatUserDate(preview.expiresAt)}
            />
          </div>
