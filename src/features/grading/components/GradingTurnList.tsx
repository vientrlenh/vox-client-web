...
/**
 * Một phần thi gồm NHIỀU lượt nói, mỗi lượt có câu hỏi + audio + transcript riêng —
 * không phải một file ghi âm duy nhất cho cả phần.
 */
export function GradingTurnList({ turns }: GradingTurnListProps) {
  if (turns.length === 0) {
    return (
  ...
  const ordered = [...turns].sort((a, b) => (a.turnOrder ?? 0) - (b.turnOrder ?? 0))
