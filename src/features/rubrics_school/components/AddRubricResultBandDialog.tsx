...
import type { AddRubricResultBandsPayload, RubricResultBandItemRequest } from '../api/useAddSchoolRubricResultBandsMutation';
...
export function AddRubricResultBandDialog({ isOpen, onClose, onSubmit, isPending, existingOrders = [] }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (const band of bands) {
      ...
      if (min >= max) {
        setErrorMessage(`Lỗi: Ở thang điểm "${band.code || band.name || '(chưa đặt tên)'}", Điểm tối thiểu (Min) phải nhỏ hơn Điểm tối đa (Max)!`);
        return;
      }

      const order = Number(band.order);
      if (seenOrders.has(order) || existingOrders.includes(order)) {
        setErrorMessage(`Lỗi: Thứ tự (Order) ${order} bị trùng lặp (ở thang điểm "${band.code || band.name || '(chưa đặt tên)'}"). Vui lòng chọn thứ tự khác.`);
        return;
      }
          ...
          <div className="space-y-4">
            {bands.map((band, index) => (
              <div key={index} className="relative space-y-3 rounded-lg border border-slate-200 p-4 pr-11">
                ...
                <p className="text-xs font-extrabold uppercase tracking-wide text-indigo-700">Thang điểm #{index + 1}</p>

                <div className="grid gap-4 sm:grid-cols-2">
                  <div>
                    <label className="mb-1 block text-sm font-bold text-slate-700">Mã Band</label>
                    <input type="text" value={band.code} onChange={(e) => updateBand(index, { code: e.target.value })} disabled={isPending} required className="w-full rounded-lg border border-slate-300 px-4 py-2 text-sm outline-none transition focus:border-indigo-500 disabled:bg-slate-50" />
                  </div>

                  <div>
                    <label className="mb-1 block text-sm font-bold text-slate-700">Tên Mức điểm</label>
                    <input type="text" value={band.name} onChange={(e) => updateBand(index, { name: e.target.value })} disabled={isPending} required className="w-full rounded-lg border border-slate-300 px-4 py-2 text-sm outline-none transition focus:border-indigo-500 disabled:bg-slate-50" />
                  </div>

                  <div className="sm:col-span-2">
                    <label className="mb-1 block text-sm font-bold text-slate-700">Mô tả</label>
                    <textarea rows={2} value={band.description} onChange={(e) => updateBand(index, { description: e.target.value })} disabled={isPending} className="w-full rounded-lg border border-slate-300 px-4 py-2 text-sm outline-none transition focus:border-indigo-500 disabled:bg-slate-50" />
                  </div>

                  <div>
                    <label className="mb-1 block text-sm font-bold text-slate-700">Điểm tối thiểu (Min)</label>
                    <input type="number" step="0.1" value={band.mappedScoreMin} onChange={(e) => updateBand(index, { mappedScoreMin: e.target.value === '' ? 0 : Number(e.target.value) })} disabled={isPending} required className="w-full rounded-lg border border-slate-300 px-4 py-2 text-sm outline-none transition focus:border-indigo-500 disabled:bg-slate-50" />
                  </div>

                  <div>
                    <label className="mb-1 block text-sm font-bold text-slate-700">Điểm tối đa (Max)</label>
                    <input type="number" step="0.1" value={band.mappedScoreMax} onChange={(e) => updateBand(index, { mappedScoreMax: e.target.value === '' ? 0 : Number(e.target.value) })} disabled={isPending} required className="w-full rounded-lg border border-slate-300 px-4 py-2 text-sm outline-none transition focus:border-indigo-500 disabled:bg-slate-50" />
                  </div>

                  <div>
                    <label className="mb-1 block text-sm font-bold text-slate-700">Thứ tự (Order)</label>
                    <input type="number" min="1" value={band.order} onChange={(e) => updateBand(index, { order: e.target.value === '' ? 1 : parseInt(e.target.value) })} disabled={isPending} required className="w-full rounded-lg border border-slate-300 px-4 py-2 text-sm outline-none transition focus:border-indigo-500 disabled:bg-slate-50" />
                  </div>
                </div>
              </div>
            ))}
