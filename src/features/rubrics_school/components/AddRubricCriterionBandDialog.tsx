// src/features/rubrics/components/AddRubricCriterionBandDialog.tsx

import { useState } from 'react';
import { X, Loader2 } from 'lucide-react';
import type { AddRubricCriterionBandsPayload } from '../api/useAddSchoolRubricCriterionBandsMutation';

type Props = {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: AddRubricCriterionBandsPayload) => Promise<void>;
  isPending: boolean;
};

export function AddRubricCriterionBandDialog({ isOpen, onClose, onSubmit, isPending }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload: AddRubricCriterionBandsPayload = {
      bands: [{ code: formData.code, scoreMin: min, scoreMax: max }],
    };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-0">
      ...
      <div className="relative w-full max-w-md rounded-xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <h2 className="text-lg font-bold text-slate-900">Thêm Mức điểm Mới</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="mt-8 flex items-center justify-end gap-3 border-t border-slate-100 pt-5">
            ...
            <button type="submit" disabled={isPending} className="inline-flex min-w-[120px] items-center justify-center gap-2 rounded-lg bg-cyan-600 px-4 py-2 text-sm font-bold text-white shadow-sm hover:bg-cyan-700 disabled:opacity-50">
              {isPending ? <Loader2 className="size-4 animate-spin" /> : 'Lưu Mức điểm'}
            </button>
          </div>
        </form>
      </div>
