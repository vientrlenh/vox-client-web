// src/features/rubrics/pages/SchoolAdminRubricCriterionBandImportPage.tsx
...
import { usePreviewSchoolRubricCriterionBandImportMutation } from '../api/usePreviewSchoolRubricCriterionBandImportMutation';
import { useAcceptSchoolRubricCriterionBandImportMutation } from '../api/useAcceptSchoolRubricCriterionBandImportMutation';
import { searchRubricCriterionBandKeys } from '../api/useSearchSchoolRubricCriterionBandsQuery';
...
// nếu không danh sách mức điểm vẫn rỗng cho tới khi người dùng tự F5.
...
export function SchoolAdminRubricCriterionBandImportPage() {
  ...
  const previewMutation = usePreviewSchoolRubricCriterionBandImportMutation(schoolId, criterionId);
  const acceptMutation = useAcceptSchoolRubricCriterionBandImportMutation(schoolId);
  ...
  async function handleAccept() {
    ...
    try {
      ...
      // danh sách mức điểm vẫn rỗng cho tới khi người dùng tự F5.
      ...
      navigate(`/school-admin/rubrics/${rubricId ?? ''}/versions/${versionId ?? ''}/criteria/${criterionId ?? ''}`);
    } catch (error) {
      setIsWaitingForResult(false);
      setMessage({
  ...
  const backToCriterionUrl = `/school-admin/rubrics/${rubricId ?? ''}/versions/${versionId ?? ''}/criteria/${criterionId ?? ''}`;

  return (
    <section aria-labelledby="school-rubric-criterionband-import-title" className="grid gap-6 font-['Be_Vietnam_Pro',sans-serif] text-slate-950">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <h1 className="mt-4 text-3xl font-bold tracking-tight text-slate-950" id="school-rubric-criterionband-import-title">
