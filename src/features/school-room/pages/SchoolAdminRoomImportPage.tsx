...
} from '../api/useSchoolRoomImportMutations'
import { schoolRoomManagementQueryKeys } from '../api/useSchoolRoomsQuery'
import type { PreviewImportResponse } from '../types'
import { formatRoomDate } from '../types'
...
export function SchoolAdminRoomImportPage() {
  ...
  async function handleAccept() {
    ...
    try {
      ...
      setMessage({
        text:
          getErrorMessage(error) ??
          'Không thể xác nhận import phòng học. Vui lòng thử lại.',
        tone: 'error',
      })
    }
  }
  ...
  return (
    <section
      aria-labelledby="school-room-import-title"
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          >
            ...
            <span>Tạo phòng học số lượng lớn</span>
          </nav>
          <h1
            className="mt-4 text-3xl font-black tracking-0 text-slate-950"
            id="school-room-import-title"
          >
            Tạo phòng học số lượng lớn
          </h1>
          <p className="mt-3 max-w-3xl text-sm font-medium leading-6 text-slate-600">
            Tải lên file CSV hoặc Excel, kiểm tra mapping cột và xác nhận import
            phòng học vào trường hiện tại. Phòng trùng mã sẽ được cập nhật tên và
            mô tả.
          </p>
        </div>
        <Link
      ...
      <section className="grid gap-4 rounded-lg border border-slate-200 bg-white p-5 shadow-sm shadow-slate-950/5">
        ...
        <label className="grid cursor-pointer place-items-center gap-3 rounded-lg border border-dashed border-slate-300 bg-slate-50 px-4 py-8 text-center transition hover:border-cyan-400 hover:bg-cyan-50">
          ...
          <span className="text-xs font-semibold text-slate-500">
            File cần có mã phòng và tên phòng.
          </span>
          <input
            accept=".csv,.xlsx,.xls"
            className="sr-only"
      ...
      {preview ? (
        <>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <SummaryCard
              label="Hết hạn"
              value={formatRoomDate(preview.expiresAt)}
            />
          </div>
