// src/features/scoring_rules_system/pages/SystemAdminScoringRulePoliciesPage.tsx
//
// Điểm vào độc lập cho Scoring Rules: vì Scoring Rule luôn là con của 1 Assessment Policy
// (không tồn tại độc lập), trang này cho System Admin chọn 1 Policy rồi mới vào quản lý
...
import {
  ...
  type SystemAssessmentPolicyFilter,
} from '@/features/assessment_policy_system/api/useSystemAssessmentPoliciesQuery';
import { formatAssessmentPolicyDate } from '@/features/assessment_policy_system';
...
export function SystemAdminScoringRulePoliciesPage() {
  const navigate = useNavigate();

  const [page, setPage] = useState(1);
  const [selectedStatus, setSelectedStatus] = useState('');

  const filter: SystemAssessmentPolicyFilter = { status: selectedStatus || null, languageId: null };
  const { data, isLoading, isError, refetch, isFetching } = useSystemAssessmentPoliciesQuery(filter, page, 10);
            ...
            <table className="w-full text-left text-sm text-slate-700">
              ...
              <tbody className="divide-y divide-slate-200">
                {policies.map((policy) => (
                  <tr
                    key={policy.id}
                    onClick={() => navigate(`/system-admin/assessment-policies/${policy.id}/scoring-rules`)}
