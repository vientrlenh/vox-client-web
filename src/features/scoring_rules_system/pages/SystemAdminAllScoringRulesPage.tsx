// src/features/scoring_rules_system/pages/SystemAdminAllScoringRulesPage.tsx
//
// Tổng hợp Scoring Rule của TẤT CẢ Assessment Policy hệ thống vào 1 bảng, kèm cột cho biết
...
import { useAllSystemScoringRulesQuery, type ScoringRuleWithPolicy } from '../api/useAllSystemScoringRulesQuery';
...
export function SystemAdminAllScoringRulesPage() {
  ...
  const { data: rules, isLoading } = useAllSystemScoringRulesQuery();
      ...
      <div className="relative flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Toàn bộ Scoring Rule của các Assessment Policy hệ thống, kèm Assessment Policy mà rule đó thuộc về.
          </p>
        </div>
      ...
      <div className="relative flex flex-col gap-3 rounded-[14px] border border-slate-200 bg-white p-4 sm:flex-row">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Tìm theo mã, tên rule, ngôn ngữ hoặc Framework Version của Assessment Policy..."
            ...
            <table className="w-full text-left text-sm text-slate-700">
              ...
              <tbody className="divide-y divide-slate-200">
                {pageRules.map((rule) => (
                  <tr
                    key={rule.id}
                    onClick={() => navigate(`/system-admin/assessment-policies/${rule.policyId}/scoring-rules`)}
