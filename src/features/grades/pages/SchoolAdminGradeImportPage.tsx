...
export function SchoolAdminGradeImportPage() {
  ...
  const backLink = gradeLevelId
    ? `/school-admin/grades/${gradeLevelId}`
    : '/school-admin/grades'
  ...
  async function handleAccept() {
    ...
    try {
      ...
      setMessage({
        text:
          getErrorMessage(error) ??
          'Không thể xác nhận import năm học. Vui lòng thử lại.',
        tone: 'error',
      })
    }
  }
    ...
    >
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          >
            <Link className="transition hover:text-blue-800" to="/school-admin/grades">
              Quản lý khối
            </Link>
            {gradeLevelId ? (
              <>
      ...
      <section className="grid gap-4 rounded-lg border border-slate-200 bg-white p-5 shadow-sm shadow-slate-950/5">
        ...
        <label className="grid cursor-pointer place-items-center gap-3 rounded-lg border border-dashed border-slate-300 bg-slate-50 px-4 py-8 text-center transition hover:border-cyan-400 hover:bg-cyan-50">
          ...
          <span className="text-xs font-semibold text-slate-500">
            File cần có mã năm học, tên năm học, ngày bắt đầu và ngày kết thúc.
          </span>
          <input
            accept=".csv,.xlsx,.xls"
            className="sr-only"
