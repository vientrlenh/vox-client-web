// src/features/assessment_policy_system/components/ArchiveRubricVersionDialog.tsx
...
// Message riêng của ArchiveSystemRubricVersionUseCase khi version đã bị cascade-archive từ trước
// (archive nốt policy active cuối cùng tự archive luôn version, xem ArchiveSystemAssessmentPolicyUseCase)
...
export function ArchiveRubricVersionDialog({
  ...
  const { data, isLoading } = useSystemAssessmentPoliciesQuery(
  ...
  const { mutateAsync: archivePolicy, isPending: isArchivingPolicies } =
    useArchiveSystemAssessmentPolicyMutation();
  const { mutateAsync: archiveVersion, isPending: isArchivingVersion } =
    useArchiveSystemRubricVersionMutation(rubricVersionId);
                ...
                {policies.map((policy) => (
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p className="mt-0.5 truncate text-xs text-slate-500">Toàn hệ thống</p>
                    </div>
                    <span
